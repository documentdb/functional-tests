"""$dateAdd type validation: null/missing, invalid field types, unit strings, argument shape."""
...
DATEADD_TYPE_SUCCESS_TESTS = DATEADD_NULL_TESTS + DATEADD_MISSING_TESTS
...
DATEADD_TYPE_TESTS: list[ExpressionTestCase] = (
    DATEADD_TYPE_SUCCESS_TESTS + DATEADD_TYPE_ERROR_TESTS + DATEADD_ARGUMENT_TESTS
)


@pytest.mark.parametrize("test_case", pytest_params(DATEADD_TYPE_TESTS))
def test_dateAdd_types(collection, test_case: ExpressionTestCase):
    """Test $dateAdd type validation and argument handling."""
