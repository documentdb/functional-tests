"""Tests for getClusterParameter parameter name semantics.

Verifies that parameter names are treated as exact literal strings:
no field-path traversal, no globbing, case-sensitive matching.
Also pins the unknown-name contract (error, not silent omission) and
verifies wildcard edge interactions in array form.
...
_BOUNDARY_NAMES: list[NameCase] = [
    NameCase(
        ...
        msg="Empty string should be treated as a literal no-match",
    ),
    NameCase(
        "whitespace_only",
        name="   ",
